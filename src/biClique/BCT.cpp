...
void BCT::buildTree() {
    ...
    while (!Q.empty()) {
        ...
        if (pivotPair.second) {
            ...
            biGraph sg = createSubgraph(SUnew, SVnew);
            ...
            for (ui i = 0; i < SVPrime.size(); i++) {
                ...
                for (ui j = 0; j < sg.pV[i + 1] - sg.pV[i]; j++) {
                    uint32_t u = sg.e2[sg.pV[i] + j];
                    SUchild.push_back(u);
                    for (ui k = sg.pU[u + 1] - 1; u >= sg.pU[u]; u--) {
                        uint32_t v = sg.e1[k];
                        if (v == v_i) {
                            break;
                        }
...
biGraph BCT::createSubgraph(const std::vector<uint32_t>& minSU, const std::vector<uint32_t>& minSV) {
    biGraph sg;
    std::unordered_map<uint32_t, uint32_t> minSUMap, minSVMap;
    uint32_t llSize = minSU.size();
    uint32_t rrSize = minSV.size();

    sg.n1 = llSize;
    sg.n2 = rrSize;
    sg.pU.resize(llSize + 1, 0);
    sg.pV.resize(rrSize + 1, 0);
    sg.e1.resize(llSize * rrSize);
    sg.e2.resize(llSize * rrSize);
