...
int uh, vh, up, vp = 0;
void BCT::processNode(Node* node) {
    std::unordered_set<uint32_t> SU = node->SU;
    std::unordered_set<uint32_t> SV = node->SV;
    ...
    if (SU.empty() && SV.empty()) {
        ...
        up = node->up;
        vp = node->vp;
        int upCap = std::min(50, up);
        int vpCap = std::min(50, vp);
        if (uh > 50 || vh > 50) return;
        for (int i = 0; i <= upCap; ++i) {
            for (int j = 0; j <= vpCap; ++j) {
                ...
                if (i + uh > 10000) printf("i+uh: %d, j+vh: %d\n", i + uh, j + vh);
                // // count[i + uh][j + vh] += 1;
                // count[0][0] = 1;
            }
        }
...
void BCT::buildTreeV2(int p_, int q_) {
    p = p_;
    q = q_;
    signal(SIGSEGV, segfault_handler);
    Node root;
    ...
    while (!Q.empty()) {
        ...
        if (SV.size() != 0 && SU.size() != 0) {
            ...
            if (pivotPair.second) {
                ...
                std::unordered_set<uint32_t> visited;
                std::unordered_set<uint32_t> commons;

                for (uint32_t v : SV) {
                    for (int i = g->pV[v]; i < g->pV[v + 1]; i++) {
                        commons.insert(g->e2[i]);
                    }
                }
    ...
    printf("Tree completed in: %ld (ms)\n", duration.count() / 1000);
    printf("Pivot time: %d\n", timePivot / 1000);
    double count = countBicliquesForPQ(&root);
    printf("Count: %f\n", count);
}
...
std::pair<uint32_t, bool> BCT::selectPivot(std::unordered_set<uint32_t>& SU, std::unordered_set<uint32_t>& SV) {
    ...
    auto End = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(End - Start);
    pivotTime += static_cast<int>(duration.count());
...
std::vector<std::vector<double>> BCT::countBicliques(Node* root) {
    ...
    auto starTravers = std::chrono::high_resolution_clock::now();
    traversePaths(root, path, allPaths);
    auto endTravers = std::chrono::high_resolution_clock::now();
    auto durationTravers = std::chrono::duration_cast<std::chrono::microseconds>(endTravers - starTravers);
    printf("Traversing completed in: %ld (ms)\n", durationTravers.count() / 1000);
...
double BCT::countBicliquesForPQ(Node* root) {
    int buf = g->maxDu * g->maxDv + 1;
    double count = 0;
    std::vector<std::vector<Node*>> allPaths;
    std::vector<Node*> path;
