...
void bcAndPath::countingV5(uint64_t T, double realV, uint32_t bar) {
...
void bcAndPath::sampleV5(std::vector<uint32_t> & nodes, uint64_t T) {
...
// fflush(stdout);
        ...
        for(int len = minPQ-1; len < minPQ && sumW > 0.5; len++) {//(len-len)-bipath
            double sumWtemp = 0.0;
            for(int j = pU[0]; j < pU[1]; j++) {
                sumWtemp += dpU[len][mapUtoV[j]];
            }

            uint64_t sampleSize = std::ceil(sumWtemp / sumW * T);
...
// printf("CY\n");
            ...
            sampleSize = std::ceil(sumWtemp / sumW * T);
            // if(ansAll[len].size() < pR) ansAll[len].resize((pR + 2)*2);
            int x = std::max(p, q) - len;
            if(q >= p) {
                if(sumCXi[x] > 0.5) 
                    ansTmp += sumCXi[x] * sumWtemp / sampleSize / C[len + x][len];
            }
            else {
                if(sumCYi[x] > 0.5) {
                    ansTmp += sumCYi[x] * sumWtemp / sampleSize / C[len + x - 1][len - 1];
                }
            }
