...
// shadow builder with decresinng order and one vertex at a time from U and 2h(u) with decreasing order for
void accuracy::shadowBuilderZStar4(int p, int q, double epsilon) {
    std::signal(SIGSEGV, segfault_handler);
    if (p > q) {
        swap(p, q);
        printf(" Working on p q e: %d - %d - %.2f \n", q, p, epsilon);
    ...
    for (uint32_t u = 0; u < g->n1; u++) {
        ...
        //     candR.changeToByPos(i, i - 1);
        // }
        // todo uncomment bellow to remove bugs
        if (candLSize < p - 1)
            continue;
        ...
        // candRSize, pqBicluqeContainZ / auxSampleSize, zInSubSpace *
        // (pqBicluqeContainZ / auxSampleSize), zInSubSpace));
        std::vector<uint32_t> candLShadow(candL.begin(), candL.begin() + candLSize);
    ...
    //     double ranPos = randomNumber * totalZinShadow;
    //     double sum = 0;
