...
// shadow builder with decreasinng order and one vertex at a time from U and 2h(u) with increasing order for
void accuracy::shadowBuilderZStar4(int p, int q, double epsilon, double delta) {
    std::signal(SIGSEGV, segfault_handler);
    if (p > q) {
        swap(p, q);
        printf(" Working on p q e: %d - %d - %.2f \n", q, p, epsilon);
    ...
    uint32_t maxE = std::min((1u << 21), g->maxDu * g->maxDv);
    std::vector<uint32_t> candLtemp(g->maxDv);
    std::vector<uint32_t> outPosition(maxE);
    vector<int> twoHopCount(g->n1);
    candL.resize(g->n1);
    candR.resize(g->n2);
    std::vector<uint32_t> candLVec, candRVec;
    ...
    std::vector<uint32_t> pV(maxE);  // Adjusted size to accommodate pV[v + 1]
    std::vector<uint32_t> pU(maxE);
    std::vector<uint32_t> e1(maxE);  // Assuming maximum possible size
    std::vector<uint32_t> e2(
        maxE);  // Assuming maximum possible sizmaxPossibleLengthe
    std::vector<double> ddp(maxE + 1, 0.0);
    std::vector<std::vector<double>> dpU(minPQ + 1,
                                         std::vector<double>(maxE + 1, 0.0));
    std::vector<std::vector<double>> dpV(minPQ + 1,
                                         std::vector<double>(maxE + 1, 0.0));
    std::vector<uint32_t> mapUtoV(maxE);
    std::vector<uint32_t> mapVtoU(maxE);
    ...
    for (uint32_t u = 0; u < g->n1; u++) {
        ...
        int maxPossibleLength = buildDP(candLSize, candRSize, p - 1, q);
        double z1 = 0, z2 = 0;
        // if (z1 < z2) {
        //     printf("success\n");
        //     printf("z1: %f, z2: %f\n", z1, z2);
        ...
        // for (uint32_t i = 0; i < pU[candLSize]; i++) {
        //     for (int j = 0; j < minPQ; j++) {
        //         printf("dpU[%d][%d]: %f\n", j, i, dpU[j][i]);
        //     }
        // }
        // printf("candLSize: %d, candRSize: %d\n", candLSize, candRSize);
        // printf("zInSubSpace: %f\n", zInSubSpace);
        // printf("zInSubSpaceTest: %f\n", zInSubSpaceTest);
        // fflush(stdout);
        ...
        uint32_t auxSampleSize = std::ceil(log(zInSubSpace));
        if (auxSampleSize < 1) {
            continue;
        }
        int auxSampleSize_temp = auxSampleSize;
        ...
        shadow.push(Subspace(p - 1, q, candLShadow, candRShadow, candLSize, candRSize,
                             pqBicluqeContainZ / auxSampleSize,
                             zInSubSpace * (pqBicluqeContainZ / auxSampleSize),
                             zInSubSpace));
    ...
    // /shadowDuration < gamma * (tTotal / auxTotalSamples) * totalZinShadow / pqBicluqeEstAux
    double constructionStop = gamma * tTotal / auxTotalSamples * totalZinShadow /
    ...
    while (shadowDuration < gamma * (tTotal / auxTotalSamples) * totalZinShadow / pqBicluqeEstAux) {
        ...
        candRR = min_space.SV;
        int minPQ2 = std::min(min_space.p_prime, min_space.q_prime);
        vector<uint32_t> pUU(maxE);
        vector<uint32_t> pVV(maxE);
        vector<uint32_t> e11(g->m);
        vector<uint32_t> e22(g->m);
        // vector<uint32_t> candLtemp2(g->maxDv * g->maxDv);
        // vector<uint32_t> outPosition2(g->maxDu * g->maxDu);
        ...
        for (uint32_t uIndex = 0; uIndex < llSize; uIndex++) {
            ...
            for (uint32_t i = pUU[uIndex]; i < pUU[uIndex + 1]; i++) {
                ...
                // sampling
                uint32_t auxSampleSize = std::ceil(log(zInMinSubSpace));
                if (auxSampleSize < 1) {
                    continue;
                }
                uint32_t auxSampleSizeTemp = auxSampleSize;
                ...
                while (auxSampleSizeTemp--) {
                    ...
                    int vCount = min_space.q_prime - min_space.p_prime + 1;
                    if (minPQ2 == 2) {
                        vCount = min_space.q_prime - min_space.p_prime;
                    }
    ...
    while (successSamples < int(gamma)) {
        ...
        for (const auto& batch : subSpaceAndC) {
            ...
            //     candRR.changeTo(s.SV[i], i);
            // }
            double zInSubSpace2 = 0;
            for (uint32_t i = 0; i < pU[llSize]; i++) {
                zInSubSpace2 += dpU[minPQ2][i];
            }
            ...
            for (int i = 0; i < batch.second; i++) {
                ...
                int vCount = s.q_prime - s.p_prime + 1;
                if (minPQ2 == 1) {
                    vCount = s.q_prime - s.p_prime;
                }
...
// shadow builder with increasing order with edge at a time (Fastest version)
void accuracy::shadowBuilderZStar3(int p, int q, double epsilon, double delta) {
    std::signal(SIGSEGV, segfault_handler);
    if (p > q) {
        swap(p, q);
        printf(" Working on p q e: %d - %d - %.2f \n", q, p, epsilon);
    ...
    for (uint32_t u = 0; u < g->n1; u++) {
        ...
        for (uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
            ...
            // printf("u: %d, v: %d, candLSize: %d, candRSize: %d\n", u, v, candLSize, candRSize);
            int maxPossibleLength = buildDP(candLSize, candRSize, p - 1, q - 1);
            ...
            double auxSampleSize = ceil(log(zInSubSpace));
            if (auxSampleSize < 1) continue;
    ...
    ////////////////////////////////////////////
    ////////////////////////////////////////////
    auto endSecondPhase = std::chrono::high_resolution_clock::now();
    double shadowDurationSecondPhase =
        std::chrono::duration_cast<std::chrono::microseconds>(endSecondPhase - start)
            .count();
    printf("shadowTotal: %f\n", shadowDurationSecondPhase);
    while (!shadow.empty()) {
        shadowVec.push_back(shadow.top());
        shadow.pop();
    }
    ...
    printf("successSamples: %d\n", successSamples);
    printf("density of biclique: %f\n", pqBicluqeContainZ / (double)t);
    printf("totalZinShadow final: %f\n", totalZinShadow);
    printf("biclique count final: %f\n", pqBicluqeContainZ / (double)t * totalZinShadow);
    printf("time: %f\n", (double)(std::chrono::high_resolution_clock::now() - start).count() / 1e9);
}

// with one edge a time and decreasing order
void accuracy::shadowBuilderZStar5(int p, int q, double epsilon, double delta) {
    std::signal(SIGSEGV, segfault_handler);
    if (p > q) {
        swap(p, q);
        printf(" Working on p q e: %d - %d - %.2f \n", q, p, epsilon);
    ...
    for (uint32_t u = 0; u < g->n1; u++) {
        ...
        for (uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
            ...
            // }
            candR.changeTo(v, --candRSize);
            std::vector<uint32_t> candRtemp2(0), candLtemp2(0);
            ...
            double auxSampleSize = log(zInSubSpace);
            if (auxSampleSize < 1) continue;
            //    / printf("sameple size: %f\n", auxSampleSize);
            int auxSampleSize_temp = auxSampleSize;
            double t_sample = 0;
...
// this includes the sampling technique of scaleGPM
void accuracy::scalGPM(int p, int q, double epsilon, double delta) {
    std::signal(SIGSEGV, segfault_handler);
    if (p > q) {
        swap(p, q);
        printf(" Working on p q e: %d - %d - %.2f \n", q, p, epsilon);
    ...
    double z = quantile(standard_normal, 1.0 - delta / 2.0);
    double epsilonTemp = 1.0;
    double mean = 0.0;
    while (epsilonTemp > epsilon) {
        ...
        for (const auto& batch : subSpaceAndC) {
            ...
            // double mu = sum / n;
            // double variance = (squaredSum / n) - (mu * mu);
            // epsilonTemp = z * (sqrt(variance / n))/mu;
            // printf("epsilonTemp: %f\n", epsilonTemp);
            // if (epsilonTemp <= epsilon) break;
            //  printf("successSamples: %d, n: %d, epsilonTemp: %f\n", successSamples, n, epsilonTemp);
            if (n % 10 == 0 && n > 1) {
                mean = (double)successSamples / n;  // = proportion of hits
                double variance = (squaredSum / n) - (mean * mean);
                double stddev = sqrt(variance / n);  // standard error of the mean
                double relative_error = z * stddev / mean;
    ...
    printf("successSamples: %d: ", successSamples);
    printf("total samples: %d\n", n);
    printf("totalZFinal: %f\n", totalZFinal);
    printf("biclique count final: %f\n", mean * totalZinShadow);
    printf("time: %f\n", (double)(std::chrono::high_resolution_clock::now() - start).count() / 1e9);
}

void accuracy::shadowBuilder_onerefinement(int p, int q, double epsilon, double delta) {
    std::signal(SIGSEGV, segfault_handler);
    if (p > q) {
        swap(p, q);
        printf(" Working on p q e: %d - %d - %.2f \n", q, p, epsilon);
    ...
    double gamma = (1 + 4 * (1 + epsilon) * (std::exp(1) - 2) * std::log(2 / delta)) /
                   (epsilon * epsilon);
    ...
    while (successSamples < int(gamma)) {
        ...
        for (const auto& batch : subSpaceAndC) {
            ...
            for (int i = 0; i < batch.second; i++) {
                ...
                vector<uint32_t> selectedL(g->maxDv + 1, 0), selectedR(g->maxDu + 1, 0);
                selectedL.clear();
                selectedR.clear();
                double point = distribution(eng);
                double temp = 0.0;
                ...
                for (int i = 1; i < minPQ2; i++) {
                    ...
                    for (int i = 0; i < selectedR.size(); i++) {
                        if (!g->connectUV(candLVec[preU], candRVec[selectedR[i]])) {
                            isBiclique = false;
                            break;
                        }
                    }
                    ...
                    if (i != minPQ2 - 1) {
                        ...
                        for (int i = 0; i < selectedL.size(); i++) {
                            if (!g->connectUV(candLVec[selectedL[i]], candRVec[preV])) {
                                isBiclique = false;
                                break;
                            }
                        }
...
//no refinements
void accuracy::shadowBuilder_noRefinement(int p, int q, double epsilon, double delta) {
    std::signal(SIGSEGV, segfault_handler);
    if (p > q) {
        swap(p, q);
        printf(" Working on p q e: %d - %d - %.2f \n", q, p, epsilon);
    ...
    std::vector<uint32_t> pV(g->n2+1);  // Adjusted size to accommodate pV[v + 1]
    std::vector<uint32_t> pU(g->n1+1);
    std::vector<uint32_t> e1(maxE);  // Assuming maximum possible size
    std::vector<uint32_t> e2(
        maxE);  // Assuming maximum possible sizmaxPossibleLengthe
    std::vector<double> ddp(maxE, 0.0);
    ...
    auto buildDP2Vec = [&](int lSize, int rSize, int p, int q) -> int {
        ...
        if (minPQ == 1) {
            for (int u = 0; u < lSize; u++) {
                for (int i = g->pU[u]; i < g->pU[u + 1]; i++) {
                    // 2 paths for vu
                    if (p == q) {
                        dpV[1][i] = g->pU[u + 1] - i - 1;
                    } else {
                        dpV[1][i] = C[g->pU[u + 1] - i - 1][q - p];
                    }
                }
            }
            return 2;
        ...
        for (; k <= minPQ && k <= minLR; k++) {
            std::fill(ddp.begin(), ddp.begin() + g->pU[lSize], 0);
            for (int v = 0; v < rSize; v++) {
                for (int i = g->pV[v] + 1; i < g->pV[v + 1]; i++) {
                    if (dpV[k - 1][mapVtoU[i]] < 0.5)
                        continue;
                    ddp[pV[v]] += dpV[k - 1][mapVtoU[i]];
                    ddp[i] -= dpV[k - 1][mapVtoU[i]];
            ...
            for (int v = 0; v < rSize; v++) {
                dpU[k][pV[v]] = ddp[g->pV[v]] < 0 ? 0 : ddp[g->pV[v]];
                for (int e = g->pV[v] + 1; e < g->pV[v + 1]; e++) {
                    dpU[k][e] = dpU[k][e - 1] + ddp[e];
                }
            }
            ...
            std::fill(ddp.begin(), ddp.begin() + pU[lSize], 0);
            for (int u = 0; u < lSize; u++) {
                for (int i = g->pU[u] + 1; i < g->pU[u + 1]; i++) {
                    if (dpU[k][mapUtoV[i]] < 0.5)
                        continue;
            ...
            for (int u = 0; u < lSize; u++) {
                ...
                for (int e = g->pU[u] + 1; e < g->pU[u + 1]; e++) {
                    dpV[k][e] = dpV[k][e - 1] + ddp[e];
                    // /printf("dpV[%d][%d]: %f\n", k+1, e, dpV[k+1][e]);
                }
            }
    ...
    uint32_t candLSize = g->n1;
    uint32_t candRSize = g->n2;
    for(uint32_t i = 0; i < g->n1; i++) {
        candL.changeTo(i,i);
    }
    for(uint32_t i = 0; i < g->n2; i++) {
        candR.changeTo(i,i);
    }
    ...
    int maxPossibleLength = buildDP(candLSize, candRSize, p , q);
    double z1 = 0, z2 = 0;
    // if (z1 < z2) {
    //     printf("success\n");
    //     printf("z1: %f, z2: %f\n", z1, z2);
    ...
    shadow.push(Subspace(p , q , candLShadow, candRShadow, candLSize, candRSize,
                         pqBicluqeContainZ / auxSampleSize,
                         zInSubSpace * (pqBicluqeContainZ / auxSampleSize),
                         zInSubSpace));
    ...
    // 0.0 and exclusive of 1.0)
    //vector<uint32_t> selectedL(g->maxDv + 1), selectedR(g->maxDu + 1);
    selectedL.resize(g->maxDv + 1);
    selectedR.resize(g->maxDu + 1);
    printf("gamma: %f\n", gamma);
    printf("tTotal: %f\n", tTotal);
    printf("auxTotalSamples: %f\n", auxTotalSamples);
    printf("totalZinShadow: %f\n", totalZinShadow);
...
void accuracy::initializeAliasMethod(std::vector<double>& probabilities,
    ...
    // Normalize and scale probabilities in-place
