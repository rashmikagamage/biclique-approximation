  ...
  auto computeDP = [&](int pL, int pR) {
    ...
    for (int v = 0; v < pR; v++) {
      for (int i = pV[v]; i < pV[v + 1]; i++) {
        int u = e2[i];
        int h =
          std::upper_bound(e1.begin() + pU[u], e1.begin() + pU[u + 1], v) -
          e1.begin();
        dpV[i][1] = pU[u + 1] - h;
      }
    }
    for (int k = 2; k < p; k++) {
      for (int u = 0; u < pL; u++) {
        for (int i = pU[u]; i < pU[u + 1]; i++) {
          int v = e1[i];
          int j =
            std::upper_bound(e2.begin() + pV[v], e2.begin() + pV[v + 1], u) -
            e2.begin();
          if (e2[j] == u)
            j++;
          dpU[i][k] = 0;
  ...
  if (sumW > 0)
    for (uint32_t u = 0; u < g->n1; u++) {
      ...
      for (uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
        ...
        ans2 += (sumCXi - preSumCXi) / std::ceil(P[i] / sumW * T) * P[i] /
          C[q - 1][p - 1];
        // printf("est %d, %u %u, sumCXi %.0f, %.0f\n", i, u, v, (sumCXi -
        // preSumCXi),
        //     (sumCXi - preSumCXi) / std::ceil(P[i] / sumW * T)/
        //     C[q-1][p-1]*P[i]);
      }
      // printf("ed\n");fflush(stdout);
    }
    ...
    for (uint32_t u = 0; u < g->n1; u++) {
      ...
      for (uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
        ...
        for (int x = 0; minPQ + x <= pR; x++) {
          ans2[x] +=
            sumCXi[x] * P[i] / sampleSize / C[minPQ + x - 1][minPQ - 1];
        }
        // ans2 += (sumCXi - preSumCXi) / std::ceil(P[i] / sumW * T) * P[i]
        // /C[maxPQ-1][minPQ-1];
...
void colorPath::approximateCountingAll(uint64_t T) {
  ...
  for (uint32_t u = 0; u < g->n1; u++) {
    ...
    for (uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
      ...
      for (int len = 2; len <= maxPLength && len < minPQ && sumW[len - 1] > 0.5;
        len++) { //(len-len)-bipath
        double sumWtemp = 0.0;
        for (int j = 0; j < pU[pL]; j++) {
        ...
        for (int x = 0; x + len <= pR + 1 && x + len < minPQ; x++) {
          ...
          //     printf("len1 %d, %d, %d\n", len, len + x,
          //     (int)ansAll[len].size()); fflush(stdout);
          // }
          // assert(len < ansAll.size());
          // assert(len + x < ansAll[len].size());
          ansAll[len][len + x] +=
            sumCXi[x] * sumWtemp / sampleSize / C[len + x - 1][len - 1];
        }

        for (int x = 1; x + len <= pL + 1 && x + len < minPQ; x++) {
          if (sumCYi[x] < 0.5)
            break;
          // if(ansAll[x + len].size() <= len) ansAll[x + len].resize((len +
          // 2)*2);
          ...
          ansAll[x + len][len] +=
            sumCYi[x] * sumWtemp / sampleSize / C[len + x - 1][len - 1];
        }
      }
    }
  }
...
void colorPath::approximateCountingAllVersion2(uint64_t T) {
  ...
  auto computeDP = [&](int pL, int pR) -> int {
    ...
    for (int u = 0; u < pL; u++) {
      for (int i = pU[u]; i < pU[u + 1]; i++) {
        ...
        e2[pV[v]] = u;
        // edges vu , stores v's N larger than u
        mapUtoV[i] = pV[v];
        // edges uv , stores u's N larger than v
        mapVtoU[pV[v]] = i;
        pV[v]++;
      }
    }
      ...
      for (int v = 0; v < pR; v++) {
        for (int i = pV[v] + 1; i < pV[v + 1]; i++) {
          if (dpV[k - 1][mapVtoU[i]] < 0.5)
            continue;
          // adding uv value to (v,u'), (v,u''), (v,u''')...
  ...
  for (uint32_t u = 0; u < g->n1; u++) {
    ...
    // candR keep neighbor list of u ie vs
    ...
    // coutning two hop neigbors of u larger than u
    for (uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
      uint32_t v = g->e1[i];
    ...
    int pLL = 0;
    // constructing candL- keps two hops of U
    ...
    for (uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
      ...
      for (; j < g->pV[v + 1]; j++) {
        uint32_t w = g->e2[j];
        // check whether the w is inside th
        if (candL.idx(w) < pLL) {
          candL.changeTo(w, pL++);
        }
      ...
      //to handle the later negibots of v 
      candR.changeTo(v, --pR);
  ...
  for (uint32_t u = 0; u < g->n1; u++) {
    ...
    for (uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
      ...
      for (int len = 2; len <= maxPLength && len < minPQ && sumW[len - 1] > 0;
        len++) { //(len-len)-bipath
        double sumWtemp = 0.0;
        for (int j = 0; j < pU[pL]; j++) {
        ...
        while (sampleSize--) {
          ...
          kk = 0;
          for (int j = 0; j < pL; j++) {
            // if(u == 3 && v == 4 && candL[stackL[0]] == 4 && candR[stackR[0]]
            // == 5) {
...
void colorPath::approximateCountingAllVersion3(uint64_t T) {
  ...
  for (uint32_t u = 0; u < g->n1; u++) {
    ...
    for (int v = 0; v < pR; v++) {
      ...
      for (int i = pV[v]; i < pV[v + 1]; i++) {
        ...
        //         i, v, pV[v], pV[v + 1], (int)e2.size(), e2[i],
        //         pL);fflush(stdout);
        // }
        int w = e2[i];
        // if(u == 38288 && 7008 == i) {
  ...
  for (uint32_t u = 0; u < g->n1; u++) {
    ...
    for (int len = 2; len < maxPLength && len < minPQ && sumW[len] > 0.5;
      len++) { //(len-len)-bipath
      double sumWtemp = 0.0;
      for (int j = pU[0]; j < pU[1]; j++) {
      ...
      for (int x = 0; x + len <= pR && x + len < minPQ; x++) {
        if (sumCXi[x] < 0.5)
          break;
        ansAll[len][len + x] +=
          sumCXi[x] * sumWtemp / sampleSize / C[len + x][len];
      }
...
void colorPath::approximateCountingAllVersion4(uint64_t T) {
  ...
  for (uint32_t u = 0; u < g->n1; u++) {
    ...
    for (uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
      ...
      // printf("\n");
      for (int len = 2; len < maxPLen && len < minPQ && sumW[len] > 0.5;
        len++) { //(len-len)-bipath
        if (dpU[len][mapUtoV[i]] < 0.5)
          break;
          ...
          for (int i = 1; i < len; i++) {
            ...
            for (int j = 0; j < i - 1; j++) {
              if (!g->connectUV(preU, stackR[j])) {
                connect = false;
                // printf("err1stackL:");
                // for(int x = 0; x < stackL.size(); x++) printf("%u ",
                // stackL[x]); printf("  "); printf("stackR:"); for(int x = 0; x
                // < stackR.size(); x++) printf("%u ", stackR[x]); printf("\n");
                break;
              }
            }
            if (!connect)
...
std::vector<std::vector<int>> getCombinations(const std::vector<int>& elements,
  int r) {
  int n = elements.size();
  ...
  while (true) {
    ...
    for (int i = 0; i < r; ++i) {
      currentCombination.push_back(elements[indices[i]]);
    }
    allCombinations.push_back(currentCombination);

    // Find the rightmost index that is not at its maximum
    int idx;
    ...
    for (int i = idx + 1; i < r; ++i) {
      indices[i] = indices[idx] + (i - idx);
    }
  }
  return allCombinations;
}

void colorPath::buildShadowFixed(int p, int q) {
  // asssuming p from U side
  // assuming p < q;
  struct ShadowPair {
  ...
  shadow.reserve(g->maxDu * g->maxDv);

  for (uint32_t u = 0; u < g->n1; u++) {
    std::vector<uint32_t> neighborsUAll;
    for (int i = g->pU[u]; i < g->pU[u + 1]; i++) {
      uint32_t v = g->e1[i];
      neighborsUAll.push_back(v);
    }
    for (uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
      ShadowPair subShadow;
      std::vector<uint32_t> neighborsU;
      // keep the neigbors of larger than v
      for (int j = i - g->pU[u] + 1; j < g->pU[u + 1]; j++) {
        neighborsU.push_back(neighborsUAll[j]);
      }
      subShadow.SV = std::move(neighborsU);
