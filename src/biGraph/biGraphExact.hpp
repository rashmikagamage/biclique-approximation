...
struct biGraph {
    ...
    biGraph(const std::string& filePath) {
        ...
        for (uint32_t i = 0; i < m; i++) {
            ...
            minV = std::min(minV, static_cast<uint64_t>(edges[i].v));  // printf("u %u, v %u\n", edges[i].u, edges[i].v);fflush(stdout);
        }
    ...
    bool connectUVFast(uint32_t u, uint32_t v) {
        // return connectUV(u, v);
        if (hashTablesL[u].n > 0)
            return hashTablesL[u].contain(v);
        else {
            return connectUV(u, v);
    ...
    bool connectVUFast(uint32_t v, uint32_t u) {
        ...
        if (hashTablesR[v].n > 0)
            return hashTablesR[v].contain(u);
        else {
            return connectVU(v, u);
