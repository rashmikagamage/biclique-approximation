...
void rawEdgePivot::addEdgesType2and3(Dinic * dinic) {
    this->dinic = dinic;
    PLtmp.resize(p);
    PRtmp.resize(q);
    PLtmp.clear();
...
void rawEdgePivot::listAllPminus1QBicliquesL(int i, int l, treePath t, uint32_t idxl) {
    if(l + t.h1 == p - 1) {
...
void rawEdgePivot::listAllPminus1QBicliquesR(int i, int r, treePath t, uint32_t idxl) {
    if(r + t.h2 == q) {
        uint32_t i = 0, j = 0;
...
void rawEdgePivot::listAllPQminus1BicliquesL(int i, int l, treePath t, uint32_t idxr) {
    if(l + t.h1 == p) {
...
void rawEdgePivot::listAllPQminus1BicliquesR(int i, int r, treePath t, uint32_t idxr) {
    if(r + t.h2 == q - 1) {
        uint32_t i = 0, j = 0;
...
void rawEdgePivot::pivotForDensestSubgraph(int l, int pL, int pR, treePath t) {
#ifdef DEBUG
