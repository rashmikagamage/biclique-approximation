...
# Command template
command_template = '../bin/run -f "{data_file}" -p {p} -q {q} -oneref'

# Timeout in seconds (10 hours)
timeout_seconds = 24 * 60 * 60

# p and q combinations
combinations = [(5, 8), (5, 10), (7, 10), (9, 6), (9, 10)]

# Number of runs per combination
num_runs = 1
# Summary CSV file
summary_csv = 'summary_oneref_24h.csv'

# Specific file names to process

# specific_file_names = [
#     "ia-wikiquote-user-edits.txt",
...
# Function to run a command with timeout and ensure the process is killed
def run_command(cmd, timeout, output_file):
    try:
        # Start the process
        with open(output_file, 'w') as out:
            process = subprocess.Popen(cmd, shell=True, stdout=out, stderr=subprocess.STDOUT)
            try:
        ...
        for p, q in combinations:
            ...
            for iteration in range(1, num_runs + 1):
                ...
                if 'timed out' in output_content.lower() or timeout_occurred:
                    ...
                    # Collect counts and times for averaging
                    count = parsed_data.get('Biclique Count Final', '')
                    time_taken = parsed_data.get('Time', '')
                    if count != '':
                        try:
            ...
            if counts and counts[0] != -1:
                ...
                avg_count = var_count = avg_time = -1  # Indicate timeout with -1
